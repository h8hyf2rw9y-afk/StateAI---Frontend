...
🚧 Real Supabase authentication (email/password + Google OAuth) and route protection, plus a real FastAPI backend behind Leads, Properties, Buyer search, Pipeline, and two AI agents (Lead Intelligence, Follow-up). The Dashboard and Appointments page still render from hand-written mock data in `features/*/mock-data.ts`. See [What's mocked](#whats-mocked-vs-real) below for the exact split.
...
- `lib/api/{leads,appointments}.ts` — typed functions (`getLeads`, `createLead`, `createAppointment`, …) matching a *speculative* backend shape (`/leads`, `/appointments` with no `/api/v1` prefix or real field names) that doesn't match how the real FastAPI backend actually models this data — **none of these are called yet**; the Appointments page still renders `features/appointments/mock-data.ts` directly. Wiring it up is a separate, smaller task now that Pipeline (which needed the same real Task/Appointment shapes, just opportunity-scoped) is real — see [Pipeline (real CRM opportunities)](#pipeline-real-crm-opportunities) and [What remains to be implemented](#what-remains-to-be-implemented).
- `lib/api/pipeline.ts` — **real, called today**: `getOpportunities`/`getOpportunity`/`updateOpportunityStage`/`getOpportunityActivities`/`getTasksForOpportunity`/`getAppointmentsForOpportunity`, all hitting the real FastAPI backend — see [Pipeline (real CRM opportunities)](#pipeline-real-crm-opportunities). This file previously held the same kind of speculative `/pipeline/deals` stub as `leads.ts`/`appointments.ts` still do — replaced, not added alongside.
...
| Appointments list page | Real UI, **mock data** (`features/appointments/mock-data.ts`) |
...
- Wiring the Appointments **list** page to the real backend instead of `features/appointments/mock-data.ts` — Leads, Properties, and Pipeline are done (see [Leads (real CRM contacts)](#leads-real-crm-contacts), [Properties (real CRM listings)](#properties-real-crm-listings), [Pipeline (real CRM opportunities)](#pipeline-real-crm-opportunities)). This is now a smaller task than it was: `lib/api/pipeline.ts`'s `getAppointmentsForOpportunity`/`getTasksForOpportunity` already prove the real `GET /appointments`/`GET /tasks` shapes work — a general Appointments page mainly needs an unfiltered list call plus its own create/edit flow, not new investigation into what the backend returns
- Wiring the Dashboard's pipeline summary / "today's priorities" / hot leads to the real Opportunities/Contacts data now that both exist (`features/dashboard/lib.ts` still derives everything from `features/pipeline/mock-data.ts`'s `mockDeals` and `features/leads/mock-data.ts`) — deliberately not done as part of making Pipeline itself real, since the Dashboard is a distinct aggregation-across-many-records problem (which opportunities/leads count as "hot" or "priority") the Pipeline task's brief didn't ask for
- A real `/pipeline/[id]` create flow, and a create flow for Tasks/Appointments generally — this task only wired up viewing an opportunity and changing its stage, matching its own brief; `POST /contacts/{id}/opportunities` exists backend-side but has no frontend form yet
- Create flows (`POST /api/v1/contacts` and `POST /api/v1/properties` already exist backend-side) — the "Add lead"/"Add property" buttons are still disabled; wiring them up is a natural next step now that both lists are real
