...
- A create/edit/cancel flow for Appointments — `POST`/`PATCH`/`DELETE /appointments/{id}` all exist backend-side, but the "Schedule appointment" button stays disabled; this task's scope was the list integration only (see [Appointments (real CRM data)](#appointments-real-crm-data))
...
- De-duplicating `features/pipeline/types.ts`'s `OpportunityAppointment` in favor of importing `AppointmentRecord` from `features/appointments/types.ts` now that the latter is the more natural, general home for this entity — a small, low-risk cleanup deliberately deferred rather than done as a side effect of the Appointments task (see that section's own note on the tradeoff)
