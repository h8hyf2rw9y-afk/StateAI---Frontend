...
- `lib/api/{leads,pipeline,appointments}.ts` — typed functions (`getLeads`, `createLead`, `moveDeal`, …) matching a *speculative* backend shape (a `/leads` REST resource) that doesn't match how the real FastAPI backend actually models this data (`/api/v1/contacts`, not `/leads`) — **none of these are called yet**; the Pipeline/Appointments pages still render `features/*/mock-data.ts` directly (Leads and Properties no longer do — see below). Wiring the rest up is a separate, larger task (see [What remains to be implemented](#what-remains-to-be-implemented)).
...
| Pipeline, Appointments list pages | Real UI, **mock data** (`features/*/mock-data.ts`) |
| Dashboard "Today's priorities" / hot leads / pipeline overview / "Available properties" count | Real UI, **derived from mock data** (`features/dashboard/lib.ts`, `features/properties/mock-data.ts`) — still mock, since it depends on Pipeline stage/score, which have no backend equivalent (see below) |
...
- Wiring the Pipeline/Appointments **list** pages to the real backend instead of `features/*/mock-data.ts` — Leads and Properties are done (see [Leads (real CRM contacts)](#leads-real-crm-contacts), [Properties (real CRM listings)](#properties-real-crm-listings)); Pipeline/Appointments need their own backend resources inspected the same way, not assumed (Pipeline in particular has no backend equivalent yet — `PipelineStage` doesn't exist on `Contact`)
...
- ✅ `npm run lint`, `npm run typecheck`, `npm run test` (79 tests), and `npm run build`.
