...
- `lib/api/{leads,properties,pipeline,appointments}.ts` — typed functions (`getLeads`, `createLead`, `moveDeal`, …) matching a *speculative* backend shape (a `/leads` REST resource) that doesn't match how the real FastAPI backend actually models this data (`/api/v1/contacts`, not `/leads`) — **none of these are called yet**; the Properties/Pipeline/Appointments pages still render `features/*/mock-data.ts` directly (Leads no longer does — see below). Wiring the rest up is a separate, larger task (see [What remains to be implemented](#what-remains-to-be-implemented)).
...
- `lib/api/errors.ts` — `getApiErrorMessage()`, the generic HTTP-status-to-safe-copy mapping shared by every real API caller (contacts, and — via `features/ai/lib.ts`'s `getAiErrorMessage`, which adds AI-specific cases on top — the AI panels too).
...
| Properties, Pipeline, Appointments list pages | Real UI, **mock data** (`features/*/mock-data.ts`) |
| Dashboard "Today's priorities" / hot leads / pipeline overview | Real UI, **derived from mock data** (`features/dashboard/lib.ts`) — still mock, since it depends on Pipeline stage/score, which have no backend equivalent (see below) |
...
- Wiring the Properties/Pipeline/Appointments **list** pages to the real backend instead of `features/*/mock-data.ts` — Leads is done (see [Leads (real CRM contacts)](#leads-real-crm-contacts)); the others need their own backend resources inspected the same way, not assumed
- A create-lead flow (`POST /api/v1/contacts` already exists backend-side) — the Leads page's "Add lead" button is still disabled; wiring it up is a natural next step now that the list itself is real
- Real pagination for the Leads list once an organization's contact count could exceed the backend's 200-per-request cap (`getContacts()` currently fetches up to 200 in one call, fine for the 20-contact demo org)
...
- ✅ `npm run lint`, `npm run typecheck`, `npm run test` (68 tests), and `npm run build`.
...
- ✅ **Real local end-to-end**: with the backend and a local Ollama (llama3.2) both running, called the exact same endpoints/paths the frontend now uses — `GET /api/v1/contacts`, `GET /api/v1/contacts/{id}`, `POST /api/v1/ai/lead-intelligence/{contact_id}`, `POST /api/v1/ai/follow-up/{contact_id}` — with a real Supabase session token against real backend demo contacts (all 20 came back from the list call, including Carlos, Gabriela, Carolina, and Sergio by name) and confirmed every JSON shape matches its TypeScript type field-for-field. Could not click through a real signed-in browser session (no browser-automation tool is available in this environment) — the request-construction side of that gap is covered by the unit tests above; a final manual click-through (sign in, open Leads, click a real contact, run both AI panels) is the one thing only a real browser can still confirm.
