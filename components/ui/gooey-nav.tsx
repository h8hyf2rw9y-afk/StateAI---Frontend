...
/**
 * "Gooey" tab switcher — origin design from RareUI, adapted here to this
 * app's own tokens (bg-muted / text-muted-foreground / the primary violet)
 * instead of RareUI's hardcoded gray/orange palette, so it reads as a native
 * PropPilot control rather than a foreign accent color.
 */
...
function Segment({ gap, span, hasSeam, leftFill, rightFill, reduced, radii, className, style, children }: SegmentProps) {
...
function NavLabel({ label, href, icon, isActive, size, activeLabelColor, onSelect }: NavLabelProps) {
  const props = {
...
export function GooeyNav({
  ...
  const [uncontrolled, setUncontrolled] = useState(() => (routeIndex === -1 ? defaultValue : routeIndex));
  ...
  const open = (seam: number) => seam === 0 || seam === items.length || seam - 1 === active || seam === active;
